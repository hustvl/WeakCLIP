    ...
    def __init__(self, layers, output_dim=1024, input_resolution=224, width=64, pretrained=None, seg_cfg=False,
        ...
        if if_fpn:
            ...
            self.fpn2 = nn.Sequential(
                nn.GroupNorm(1, width * 8),
                nn.Conv2d(width * 8, width * 8, kernel_size=3, padding=1),
            )

            self.fpn3 = nn.GroupNorm(1, width * 16)

            self.fpn4 = nn.Sequential(
                nn.GroupNorm(1, width * 32),
                # nn.MaxPool2d(kernel_size=2, stride=2)
                nn.Conv2d(width * 32, width * 32, kernel_size=3, padding=1),
            )
...
class CLIPVisionTransformer(nn.Module):
    def __init__(self, input_resolution=224, patch_size=32, width=768, layers=12, heads=12, output_dim=512, drop_path_rate=0.0, out_indices=[3, 5, 7, 11], pretrained=None, get_embeddings=False, if_fpn=True, **kwargs):
        super().__init__()
        self.pretrained = pretrained
        self.input_resolution = input_resolution
        self.output_dim = output_dim
...
class CLIPTextContextEncoder(nn.Module):
    ...
    def forward(self, text, context=None):
        if context != None:
            ...
            # if self.if_fix_adapter:
            #     x = x.clone().data.detach()
            x = x + self.positional_embedding
            x = x.permute(1, 0, 2)  # NLD -> LND
            x = self.transformer(x)
            x = x.permute(1, 0, 2)  # LND -> NLD
